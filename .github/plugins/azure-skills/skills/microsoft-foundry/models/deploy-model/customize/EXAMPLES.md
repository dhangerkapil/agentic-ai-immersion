...
**Scenario:** Deploy gpt-5.4 accepting all defaults for quick setup.
**Config:** gpt-5.4 / GlobalStandard / 10K TPM / Dynamic Quota enabled
**Result:** Deployment `gpt-5.4` created in ~2-3 min with auto-upgrade enabled.

## Example 2: Production Deployment with Custom Capacity

**Scenario:** Deploy gpt-5.4 for production with high throughput.
**Config:** gpt-5.4 / GlobalStandard / 50K TPM / Dynamic Quota / Name: `gpt-5.4-production`
...
**Scenario:** Deploy gpt-5.4 with reserved capacity (PTU) for predictable workload.
**Config:** gpt-5.4 / ProvisionedManaged / 200 PTU (min 50, max 1000) / Priority Processing enabled
...
**Scenario:** Deploy gpt-5.4-mini for dev/testing with minimal cost.
**Config:** gpt-5.4-mini / Standard / 1K TPM / Name: `gpt-5.4-mini-dev`
**Result:** 1K TPM, 10 req/10s. Minimal pay-per-use cost for development and prototyping.

## Example 5: Spillover Configuration

**Scenario:** Deploy gpt-5.4 with spillover to handle peak load overflow.
**Config:** gpt-5.4 / GlobalStandard / 20K TPM / Dynamic Quota / Spillover → `gpt-5.4-backup`
...
|----------|-------|-----|----------|:---:|:---:|:---:|----------|
| Ex 1 | gpt-5.4 | GlobalStandard | 10K TPM | ✓ | - | - | Quick setup |
| Ex 2 | gpt-5.4 | GlobalStandard | 50K TPM | ✓ | - | - | Production |
| Ex 3 | gpt-5.4 | ProvisionedManaged | 200 PTU | - | ✓ | - | Predictable workload |
| Ex 4 | gpt-5.4-mini | Standard | 1K TPM | - | - | - | Dev/testing |
| Ex 5 | gpt-5.4 | GlobalStandard | 20K TPM | ✓ | - | ✓ | Peak load |
...
|-------|-------|-----|----------|--------|
| Dev | gpt-5.4-mini | Standard | 1K TPM | — |
| Staging | gpt-5.4 | GlobalStandard | 10K TPM | — |
| Production | gpt-5.4 | GlobalStandard | 50K TPM | Dynamic Quota + Spillover |

### Cost Optimization

- **High priority:** gpt-5.4, ProvisionedManaged, 100 PTU, Priority Processing
- **Low priority:** gpt-5.4-mini, Standard, 5K TPM
...
**Cost:** Right-size capacity; use gpt-5.4-mini where possible (80-90% accuracy at lower cost); enable dynamic quota; consider PTU for consistent high-volume.
...
| Version not available for SKU | Check `az cognitiveservices account list-models --query "[?name=='gpt-5.4'].version"`, use latest |
| Deployment name exists | Skill auto-generates unique name (e.g., `gpt-5.4-2`), or specify custom name |
